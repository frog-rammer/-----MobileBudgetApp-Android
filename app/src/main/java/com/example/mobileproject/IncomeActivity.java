...
public class IncomeActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //수입 리스트 -----DB에서
        final ArrayList<String> incomeList = new ArrayList<String>();
        ListView list = findViewById(R.id.listIncome);
        ArrayAdapter<String> adapter = new ArrayAdapter<String>(this, android.R.layout.simple_list_item_1, incomeList);
        list.setAdapter(adapter);
        Cursor cursor = ((MainActivity)MainActivity.mContext).IncomeList();
        ...
        daybtn_back.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                totalupdate(date);
                update(date, incomeList, adapter, selected_category);
            }
        });
        ...
        btnCategory.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                showCategory(incomeList, adapter);}
        });

    }
    private void showCategory(ArrayList incomeList, ArrayAdapter adapter){
        AlertDialog.Builder dlg_category = new AlertDialog.Builder(this);
        LayoutInflater inflater = (this).getLayoutInflater();
        View dialogView= inflater.inflate(R.layout.dialog_income_category, null);
        dlg_category.setView(dialogView);
        //data source
        final CharSequence[] categoryList = {"전체","월급","용돈","공모전 상금","복권","상여금","기타"};
        dlg_category.setTitle("수입 입력");
    ...
    public void totalupdate() {
        ((MainActivity)MainActivity.mContext).TotalIncome();
        SharedPreferences sharedPreferences = getSharedPreferences("sharedPreferences", 0);
        txtTotal.setText(String.valueOf(sharedPreferences.getInt("today_income", 0)));
    }
    ...
    public void update(String date, ArrayList incomeList, ArrayAdapter adapter, String item) {
        Cursor cursor = ((MainActivity)MainActivity.mContext).IncomeList(date, item);
